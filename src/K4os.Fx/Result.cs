...
public static class ResultExtensions
{
	...
	public static Option<V> TryGetValue<K, V>(this IDictionary<K, V> map, K key) =>
		map.TryGetValue(key, out var result) ? Option.Some(result) : Option.None<V>();
}
...
public readonly struct Result<T>
{
	...
	public T OrFail(Exception? exception = null) =>
		_error switch { null => _value!, var e => throw exception ?? e.Rethrow(), };
